...
const HomePageCover = () => {
	...
	let fetchSiteMetadata = async () => {
		let res = await apiGET(`/v1/site-metadata/get-site-metadata-by-type?type=${"belowBannner"}`);
		if (res?.status == 200) setSiteData(res.data.data.data);
	};

	let fetchSiteMetaDataSmallHeading = async () => {
		let res = await apiGET(`/v1/site-metadata/get-site-metadata-by-type?type=${"belowBannnerSmallText"}`);
	...
	return (
		<div className="w-100">
			<LazyLoadImage effect="blur" src={banner} className="banner img-fluid coverPic" alt="Banner" />
			{siteData?.length > 0 && (
			...
			{siteDataSmallHeading?.length > 0 && (
				<div className="fs-5 text-center mt-3 subHeadingHome px-2">
					{siteDataSmallHeading?.length > 0 && siteDataSmallHeading[0]?.statements?.length
